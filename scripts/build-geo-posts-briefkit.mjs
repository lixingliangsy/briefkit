// build-geo-posts-briefkit.mjs
// Reads GEO article markdown files from opc-doc (briefkit), extracts frontmatter + JSON-LD +
...
const ARTICLES_DIR = 'E:/AgentCPM/07_一人公司出海项目/opc-doc/outputs/07-conversion/articles-briefkit'
const OUT_TS = 'E:/AgentCPM/07_一人公司出海项目/12_Micro_SaaS出海/briefkit/data/geoPosts.ts'
// NOTE: Vercel alias is NOT briefkit.vercel.app — after first deploy, replace both with the
// real alias (e.g. briefkit-<random>.vercel.app) and re-run this script + redeploy.
const PROD_URL = 'https://briefkit.lxsaihub.com'
const LIVE_DOMAIN = 'https://briefkit.lxsaihub.com'
...
function parseCta(rest) {
  ...
  if (!line) return ''
  const text = line.replace(/\[WAITLIST_URL\]/g, `<a href="${PROD_URL}" class="underline font-semibold">try BriefKit</a>`)
  return '<p>' + inline(text) + '</p>'
}

function fixDomain(obj) {
  return JSON.parse(JSON.stringify(obj).replace(/https:\/\/briefkit\.vercel\.app/g, LIVE_DOMAIN))
}
...
for (const f of files) {
  ...
  const description = (blogPosting.description || '').replace(/https:\/\/briefkit\.vercel\.app/g, LIVE_DOMAIN)
  posts.push({
    slug: fm.slug,
    ...
    description,
    html: mdToHtml(body).replace(/https:\/\/briefkit\.vercel\.app/g, LIVE_DOMAIN),
    faq: parseFaq(faq),
...
const ts = `// AUTO-GENERATED by scripts/build-geo-posts-briefkit.mjs — do not edit by hand.
